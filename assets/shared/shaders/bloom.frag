...
float SCurveGay (float value, float amount, float correction) {
	float curve = 1.0;
...
vec3 chromaticAbberation(sampler2D texgay, vec2 uvgay, float amount) {
	float aberrationAmount = amount/10.0;
   	vec2 distFromCenter = uvgay - 0.5;
	...
	for (int i = 1; i <= 8; i++) {
		...
		color.r += flixel_texture2D(texgay, uvgay - float(i) * aberrated).r * weight;
		color.b += flixel_texture2D(texgay, uvgay + float(i) * aberrated).b * weight;
	}
	
	color.g = flixel_texture2D(texgay, uvgay).g * 0.9961; // 0.9961 = weight(1)+weight(2)+...+weight(8);
...
{
	return vec3(SCurveGay(color.r, 3, 1), 
				SCurveGay(color.g, 6, 1), 
				SCurveGay(color.b, 5, 1)
			   );
}
...
vec3 flares(sampler2D texgay, vec2 uvgay, float threshold, float intensity, float stretch, float brightness) {
	threshold = 1.0 - threshold;
	
	vec3 hdr = flixel_texture2D(texgay, uvgay).rgb;
	...
	{
		float texL = flixel_texture2D(texgay, uvgay+vec2(i/d, 0.0)).r;
		float texR = flixel_texture2D(texgay, uvgay-vec2(i/d, 0.0)).r;
	...
	{
		float texU = flixel_texture2D(texgay, uvgay+vec2(0.0, i/d)).r;
		float texD = flixel_texture2D(texgay, uvgay-vec2(0.0, i/d)).r;
...
vec3 highlights(vec3 pixelgay, float thresgay)
{
	float val = (pixelgay.x + pixelgay.y + pixelgay.z) / 3.0;
	return pixelgay * smoothstep(thresgay - 0.1, thresgay + 0.1, val);
}
...
vec3 glow(vec3 colgay, vec2 uvgay)
{
	vec3 colorgay = blur(colgay, uvgay, 2.0);
	colorgay += blur(colgay, uvgay, 3.0);
	colorgay += blur(colgay, uvgay, 5.0);
	colorgay += blur(colgay, uvgay, 7.0);
	colorgay /= 4.0;
	
	colorgay += samplef(uvgay, colgay);
...
void main() {
	...
	vec3 colorgay = flixel_texture2D(bitmap, uvgay).xyz;
	...
	//contrast
	colorgay = contrast(colorgay) * 0.9;
	
	
	//flare
	colorgay += flares(bitmap, uvgay, 0.9, 200.0, .04, 0.1);
	
	
	//output
	gl_FragColor = vec4(colorgay, 1.0);
}