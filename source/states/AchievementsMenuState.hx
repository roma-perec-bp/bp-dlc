...
class AchievementsMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		nameText.setFormat(Paths.font("mariones.ttf"), 32, FlxColor.WHITE, CENTER);
		nameText.scrollFactor.set();

		descText = new FlxText(50, nameText.y + 38, FlxG.width - 100, "", 24);
		descText.setFormat(Paths.font("mariones.ttf"), 24, FlxColor.WHITE, CENTER);
		...
		progressTxt.setFormat(Paths.font("mariones.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		if(Achievements.isUnlocked('friday_night_play') && Achievements.isUnlocked('unfuckable_beat') && Achievements.isUnlocked('unfuckable_lose')
			&& Achievements.isUnlocked('unfuckable_nodeaths') && Achievements.isUnlocked('unfuckable_nomiss') && Achievements.isUnlocked('unfuckable_finalwave')
			&& Achievements.isUnlocked('unfuckable_dontmiss') && Achievements.isUnlocked('death_insta') && Achievements.isUnlocked('exerection_nomiss') && Achievements.isUnlocked('rookie_nomiss')
			&& Achievements.isUnlocked('dep_nomiss') && Achievements.isUnlocked('dep_lose') && Achievements.isUnlocked('caramelldansen_nomiss')  && Achievements.isUnlocked('caramelldansen_chart')
			&& Achievements.isUnlocked('hard-mode_lose') && Achievements.isUnlocked('hard-mode_nomiss') && Achievements.isUnlocked('secretsongs')  && Achievements.isUnlocked('holy-hell_beat')
			&& Achievements.isUnlocked('holy-hell_beat') && Achievements.isUnlocked('holy-hell_hard') && Achievements.isUnlocked('holy-hell_lose')  && Achievements.isUnlocked('end')
			&& Achievements.isUnlocked('chart') && Achievements.isUnlocked('pizdec') && Achievements.isUnlocked('cum') && Achievements.isUnlocked('downscroll') && !Achievements.isUnlocked('onehundrec'))
		{
			FlxG.sound.music.stop();
			MusicBeatState.switchState(new GetALifeState());
		}
	}
	...
	function _changeSelection()
	{
		...
		if(options[curSelected].hidden && options[curSelected].unlocked)
			descText.text = options[curSelected].description;
		else if(options[curSelected].hidden && !options[curSelected].unlocked)
			descText.text = '???';
		else
			descText.text = options[curSelected].description;
...
class ResetAchievementSubstate extends MusicBeatSubstate
{
	...
	public function new()
	{
		...
		text.setFormat(Paths.font("mariones.ttf"), 40, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		text.scrollFactor.set();
		text.borderSize = 2;
		add(text);
