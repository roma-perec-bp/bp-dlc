...
class LoadingState extends MusicBeatState
{
	...
	override function create()
	{
        ...
        bar = new FlxSprite(bg.x + 5, bg.y + 5).makeGraphic(1, 1, FlxColor.RED);
        bar.scale.set(0, 15);
        bar.updateHitbox();
        add(bar);
	...
	{
		if(!ClientPrefs.data.loadingScreen || ClientPrefs.data.optimize) intrusive = false;
	...
	{
		if(ClientPrefs.data.multithread)
			threadPool = new FixedThreadPool(#if cpp getCPUThreadsCount() #else 8 #end);
		else
			threadPool = new FixedThreadPool(1);
	}
		...
		.then((_) -> new Future<Bool>(() -> {
			...
			if (!dontPreloadDefaultVoices && prefixVocals != null)
			{
				if (PlayState.SONG.song == 'UNFUCKABLE')
				{
                    ...
                    songsToPrepare.push('$prefixVocals-Beatus');
				}
				else
				{
			...
			if (PlayState.SONG.song == 'UNFUCKABLE')
            {
                threadsMax++;
                threadPool.run(() -> {
                    try { preloadCharacter('unfuck_pole'); } catch (e:Dynamic) {}
                    completedThread();
                });
            }
            if (PlayState.SONG.song == 'UNFUCKABLE')
            {
                threadsMax++;
                threadPool.run(() -> {
                    try { preloadCharacter('unfuck_jack'); } catch (e:Dynamic) {}
                    completedThread();
                });
            }
