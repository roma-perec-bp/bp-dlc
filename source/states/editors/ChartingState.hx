...
class ChartingState extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent
{
	...
	[
		...
		['Play Uninterruptable Animation', "Plays an animation on a Character,\nonce the animation is completed,\nthe animation changes to Idle\n\nValue 1: Animation to play.\nValue 2: Character (Dad, BF, GF)"],
		['Shake Beat', 'Toogles shake on beat'],
		...
		['Change Icon', "Value 1: Character to change (DAD, BF, GF)\nValue 2: Icon's name\nValue 3: Health bar color change"],
		...
		['Fade Camera', "Value 1: Color, Value 2: Duration, Value 3: Camera"],
		['BG behind', "Value 1: Alpha, Value 2: Duration, Value 3: Color"],
		['Set Health', "Value 1: value"],
		['Remove Health Limit', "Value 1: value"],
		['Add Health', "Value 1: value - or +"],
		['Set Health Tween', "value 1: Value\n value 2: Time\nValue 3: Ease"],
		...
		['Update Vocals', "Put it to update vocal volume if bf missed"],
		['Change Stage', "USEFUL ONLY IF THERE IS CODE OF CHANGING\nValue 1: Put here text of stage name and boom\nOther Values can be extras if coded"],
		...
		['Glitch Small', 'Value 1: Int'],
		['Glitch Big', 'Value 1: Int'],
		['Swap Icon', "Value 1: Character to swap (PEP, POLE, JACK)"],
		['Faking Icons', "does thing"],
		['Dad Icon Bye', "does thing bye bye dad"],
		['DONT MISS', 'Triggers NOT MISS, YOU SUCK'],
		['BACKDROP GUYS', 'gay'],
		['BACKDROP HWAW', 'ew'],
		['Ghost Behind', 'on value 1 put 0 if u dont wanna see dDAD'],
		['We Are Brutal Ex', 'toggles pepper we are brutal ex'],
		['CapCut Boom', 'BNAGER'],
		['Zombie Eat', 'type on if u wanna get him'],
		['Zombie Gimmick', 'type beat'],
		['Update Strum', 'you heard it'],
		['bg hypno', 'it toggles epilepcy'],
		['Jackson Activate', 'enables jacson'],
		['Jackson Stop', 'disables jacksos'],
		['Jackson Hud', 'spawns jackson on hud'],
	...
	function _cacheSections()
	{
		...
		if(FlxG.sound.music != null && !reachedLimit) //Created sections to fill blank space
		{
			...
			var bf:Bool = lastSection != null ? lastSection.hwaw : true;
			var dad:Bool = lastSection != null ? lastSection.pepper : false;
			var jackson:Bool = lastSection != null ? lastSection.jack : false;
			var zondrie:Bool = lastSection != null ? lastSection.pole : false;
	...
	function loadSection(?sec:Null<Int> = null)
	{
		...
		if(sec != null)
		{
			...
			perecAl.checked = sec.pepper;
			hwawAl.checked = sec.hwaw;
			poleAl.checked = sec.pole;
			jacksonAl.checked = sec.jack;
		}
	...
	var perecAl:PsychUICheckBox;
	var hwawAl:PsychUICheckBox;
	var poleAl:PsychUICheckBox;
	var jacksonAl:PsychUICheckBox;
	...
	function addCameraTab()
	{
		...
		perecAl = new PsychUICheckBox(objX, objY, 'Pepper Visible?', 70, function()
		{
			var sec = getCurChartSection();
			if(sec != null) sec.pepper = perecAl.checked;
		});
		hwawAl = new PsychUICheckBox(objX + 100, objY, 'Hwaw Visible?', 70, function()
		{
			var sec = getCurChartSection();
			if(sec != null) sec.hwaw = hwawAl.checked;
		});
		poleAl = new PsychUICheckBox(objX + 200, objY, 'Zombie Pole Visible?', 70, function()
		{
			var sec = getCurChartSection();
			if(sec != null) sec.pole = poleAl.checked;
		});
		jacksonAl = new PsychUICheckBox(objX + 300, objY, 'Jackson Visible?', 70, function()
		{
			var sec = getCurChartSection();
			if(sec != null) sec.jack = jacksonAl.checked;
		});
		...
		tab_group.add(perecAl);
		tab_group.add(hwawAl);
		tab_group.add(poleAl);
		tab_group.add(jacksonAl);
